...
// ---------------------------------------------------------------------------
// RECONSTRUCTED FROM HERE DOWN. Everything above this comment (validation up
// through the email regex, and the idempotent-lookup branch above it) is
// byte-for-byte what the tool recovered from the live deployment before
// truncating; everything below is a best-effort rebuild of the missing
// "create the PayPal invoice draft" step, inferred from PayPal's Invoicing
// v2 API and the exact `/send` call this file already makes (unchanged,
// verbatim, further down). It has NOT been exercised against PayPal and
// should be run against the sandbox (PAYPAL_MODE=sandbox) before any live use.
// ---------------------------------------------------------------------------
const value=deposit>0?deposit:(amount>0?amount:contract);
if(!Number.isFinite(value)||value<=0)return res.status(400).json({ok:false,error:"invalid_amount"});
const cr=await pp("/v2/invoicing/invoices",{method:"POST",headers:{"PayPal-Request-Id":"create-deposit-"+slot},body:JSON.stringify({
  detail:{invoice_number:invNo,currency_code:"USD",reference:deal,note:"Deposit invoice for "+project},
...
const sr=await pp("/v2/invoicing/invoices/"+encodeURIComponent(id)+"/send",{method:"POST",headers:{"PayPal-Request-Id":"send-deposit-"+slot},body:JSON.stringify({send_to_recipient:true,send_to_invoicer:true})});
